...
public class GetProjectCostSummaryQueryHandlerTests
{
    ...
    public async Task HandleAsync_ShouldConsolidateCostsByType()
    {
        ...
        db.Expenses.AddRange(
            new Expense { WorkProjectId = projectId, CostType = CostType.Labor, Amount = 100m, Description = "A" },
            new Expense { WorkProjectId = projectId, CostType = CostType.Labor, Amount = 50m, Description = "B" },
            new Expense { WorkProjectId = projectId, CostType = CostType.Utility, Amount = 30m, Description = "C" });
